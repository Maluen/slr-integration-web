...
export default class ProjectAccessesActions {
  ...
  fetch(projectId) {
    return this.alt.defer(async (resolve) => {
      try {
        ...
        const response = await Globals.services.get('readProjectAccesses', { projectId, req: this.alt.req });
  ...
  deleteProjectAccess(id) {
    return this.alt.promise(async (resolve) => {
      try {
        const response = await Globals.services.post('deleteProjectAccess', { id, req: this.alt.req });
